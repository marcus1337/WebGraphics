...
void IOOBJ::loadModel(std::string path, std::string name) {
    ...
    modelDataMap[name] = readModelData(reader);
}

ModelData IOOBJ::readModelData(tinyobj::ObjReader& reader) {
    ModelData modelData;
    ...
    for (size_t s = 0; s < shapes.size() && s < 1; s++) { //s < 1 because only loading one shape per file, currently.
        ...
        for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
            ...
            for (size_t v = 0; v < fv; v++) {
                ...
                addMeshIndex(reader, idx, modelData);
            }
            index_offset += fv;
        }
    }
...
void IOOBJ::addMeshIndex(tinyobj::ObjReader& reader, tinyobj::index_t& idx, ModelData& data) {
    auto& attrib = reader.GetAttrib();
    glm::vec3 position = glm::vec3(attrib.vertices[3 * size_t(idx.vertex_index) + 0],
        attrib.vertices[3 * size_t(idx.vertex_index) + 1],
        attrib.vertices[3 * size_t(idx.vertex_index) + 2]);
    data.positions.push_back(position);

    if (idx.normal_index >= 0) {
        glm::vec3 normal = glm::vec3(attrib.normals[3 * size_t(idx.normal_index) + 0],
            attrib.normals[3 * size_t(idx.normal_index) + 1],
            attrib.normals[3 * size_t(idx.normal_index) + 2]);
        data.normals.push_back(normal);
    }
    if (idx.texcoord_index >= 0) {
        glm::vec2 texCoord = glm::vec2(attrib.texcoords[2 * size_t(idx.texcoord_index) + 0],
